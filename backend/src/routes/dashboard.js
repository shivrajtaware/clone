...
router.get('/', async (req, res) => {
  ...
  const [recentPatients, todayAppts, announcements, pendingLabOrders, lowStockItems] = await Promise.all([
    ...
    prisma.appointment.findMany({ where: { hospital_id: req.hospitalId, appointment_date: { gte: today, lt: tomorrow }, status: { notIn: ['CANCELLED'] } }, take: 10, orderBy: { slot_time: 'asc' }, include: { patient: { select: { first_name: true, last_name: true } }, doctor: { select: { first_name: true, last_name: true } } } }),
